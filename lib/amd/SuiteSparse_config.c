...
void SuiteSparse_config_free(void* p) { SuiteSparse_config.free_func(p); }
...
void SuiteSparse_finish(void) { /* do nothing */ ; }
...
int SuiteSparse_divcomplex(double xr,
                           double xi, // real and imaginary parts of x
                           double yr,
                           double yi, // real and imaginary parts of y
                           double* zr,
                           double* zi // real and imaginary parts of z
)
{
  double tr, ti, r, den;
...
const char* SuiteSparse_BLAS_library(void)
{
  ...
  return ((sizeof(SUITESPARSE_BLAS_INT) == 8) ? "OpenBLAS (64-bit integers)"
                                              : "OpenBLAS (32-bit integers)");
#elif defined(BLAS_FLAME)
  return ((sizeof(SUITESPARSE_BLAS_INT) == 8) ? "FLAME (64-bit integers)"
...
#else
  return ((sizeof(SUITESPARSE_BLAS_INT) == 8) ? "Other BLAS (64-bit integers)"
