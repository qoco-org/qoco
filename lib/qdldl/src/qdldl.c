 ...
 *
 * Copyright 2018, Paul Goulart, Bartolomeo Stellato, Goran Banjac, The OSQP
 * developers
 *
...
QDLDL_int QDLDL_factor(const QDLDL_int n, const QDLDL_int* Ap,
                       ...
                       QDLDL_int pos_diags, QDLDL_float dyn_reg)
{
  ...
  for (k = 1; k < n; k++) {
    ...
    for (i = Ap[k]; i < tmpIdx; i++) {
      ...
      if (yMarkers[nextIdx] == QDLDL_UNUSED) { // this y term not already
                                               // visited
...
void QDLDL_solve(const QDLDL_int n, const QDLDL_int* Lp, const QDLDL_int* Li,
